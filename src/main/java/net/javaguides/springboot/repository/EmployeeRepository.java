...
public interface EmployeeRepository extends JpaRepository<Employee, Long> {
    ...
    Employee findByJPQLWithNamedParams(@Param("firstName") String firstName,
                                       @Param("lastName") String lastName);

    @Query(value = "SELECT * FROM employees e " +
            "WHERE e.first_name = ?1 " +
            "AND e.last_name = ?2", nativeQuery = true)
    Employee findByNativeSQLWithIndexParams(String firstName, String lastName);

    @Query(value = "SELECT * FROM employees e " +
            "WHERE e.first_name = :firstName " +
            "AND e.last_name = :lastName", nativeQuery = true)
    Employee findByNativeSQLWithNamedParams(@Param("firstName")String firstName,
                                            @Param("lastName") String lastName);
}
